...
class ProductDetailsViewModel {
    // MARK:- Variables
    var responseData : ShowProductDetails?
    
    //MARK:- Functions and Methods
    func getProductDetails(product_id:String, completionHandler:@escaping (ShowProductDetails)->Void)
    {
        var url1 = URLComponents(string: STRING.getProductDetailURL)
        url1!.queryItems = [
            URLQueryItem(name: STRING.product_id, value: "\(product_id)")
        ]
        ...
        api.setupPostMethod(urlFromModel:url, parameter:para, method:STRING.httpGetMethod, header:header){ (result) in
            ...
            case .Success(let data):
                ...
                do{
                    self.responseData = try JSONDecoder().decode(ShowProductDetails.self, from: data2)
